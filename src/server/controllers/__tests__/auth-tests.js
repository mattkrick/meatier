...
test('AuthController:sendResetEmail:Success', async t => {
  t.plan(1);
  const app = makeApp();
  await request(app)
    .post('/auth/signup')
    .send({email: 'AuthControllerSendResetEmail@Success', password: '123123'});
  const res = await request(app)
    .post('/auth/send-reset-email')
    .send({email: 'AuthControllerSendResetEmail@Success'})
  t.is(res.status, 200);
});

test('AuthController:sendResetEmail:EmailSchemaError', async t => {
  t.plan(2);
  const app = makeApp();
  const res = await request(app)
    .post('/auth/send-reset-email')
...
test('AuthController:sendResetEmail:EmailNotFound', async t => {
  t.plan(2);
  const app = makeApp();
  const res = await request(app)
    .post('/auth/send-reset-email')
    .send({email: 'AuthControllerSendResetEmail@EmailNotFoundX'})
  t.is(res.status, 401);
